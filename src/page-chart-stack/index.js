...
import { max, select, event, mouse, scaleOrdinal, scaleBand, scaleLinear, axisBottom, axisLeft, stack, stackOrderNone, stackOffsetNone } from "d3";
...
class ShapeChart extends Component {
  ...
  @action parentData = () => {
    const org = toJS(store.stackData)
      .reduce((acc, cur) => [...acc, ...Object.keys(cur)], [])
      .filter((element, index, array) => element !== "month" && array.indexOf(element) === index);
    const month = toJS(store.stackData).reduce((acc, cur) => (acc.includes(cur.month) ? acc : [...acc, cur.month]), []);
  ...
  createBarChart = () => {
    ...
    barRect
      ...
      .on("mousemove", function (d) {
        const left =
          event.screenX + document.getElementById("stack-tooltip").offsetWidth + 30 > window.innerWidth ? mouse(this)[0] - document.getElementById("stack-tooltip").offsetWidth : mouse(this)[0] + 30;
